-- ===========================================================================
include( "Akane_CitySupport_Expansion2" );
...
local function GetAkaneQueueDistrictIconName(districtType:string)
...
local function GetAkaneQueueUnitPortraitCandidates(playerID:number, pUnitDef:table)
	if pUnitDef ~= nil and pUnitDef.UnitType == AKANE_UNIT_STAGE_ACTOR then
		local pRockBandDef:table = GameInfo.Units["UNIT_ROCK_BAND"];
		if pRockBandDef ~= nil then
			local iconName, prefixOnlyIconName, eraOnlyIconName, fallbackIconName = GetUnitPortraitIconNamesFromDefinition(playerID, pRockBandDef);
...
function CreateQueueInstance( instanceManager:table, parentControl:table, playerID:number, cityID:number, queueIndex:number, queueEntry:table )
	...
	if queueEntry then
		...
		if queueEntry.Directive == CityProductionDirectives.TRAIN then
			...
			kPossibleIcons = GetAkaneQueueUnitPortraitCandidates(playerID, pUnitDef);
		...
		elseif queueEntry.Directive == CityProductionDirectives.ZONE then
			...
			kPossibleIcons = {GetAkaneQueueDistrictIconName(pDistrictDef.DistrictType)};
