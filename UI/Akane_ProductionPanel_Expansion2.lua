...
include( "Akane_ProductionHelper" );
...
local function GetAkaneProductionListIconType(itemType:string)
	if itemType == AKANE_DISTRICT_LALALAI_TROUPE then
		return "DISTRICT_THEATER";
	end
	if itemType == AKANE_UNIT_STAGE_ACTOR then
		return "UNIT_ROCK_BAND";
	end
	return itemType;
end
...
function PopulateGenericItemData( kInstance:table, kItem:table )
	...
	-- Icon
	kInstance.Icon:SetIcon(ICON_PREFIX..GetAkaneProductionListIconType(kItem.Type));
