...
local AKANE_DISTRICT_LALALAI_TROUPE:string = "DISTRICT_LALALAI_TROUPE";
local AKANE_UNIT_STAGE_ACTOR	:string = "UNIT_STAGE_ACTOR";

local function GetAkaneProductionDistrictIcons(districtType:string)
	if districtType == AKANE_DISTRICT_LALALAI_TROUPE then
		return {"ICON_DISTRICT_THEATER"};
	end
	return {"ICON_" .. districtType};
end

local function GetAkaneProductionUnitIcons(owner:number, unitDef:table)
	if unitDef ~= nil and unitDef.UnitType == AKANE_UNIT_STAGE_ACTOR then
		local pRockBandDef:table = GameInfo.Units["UNIT_ROCK_BAND"];
		if pRockBandDef ~= nil then
			local iconName, prefixOnlyIconName, eraOnlyIconName, fallbackIconName = GetUnitPortraitIconNamesFromDefinition(owner, pRockBandDef);
			return {iconName, prefixOnlyIconName, eraOnlyIconName, fallbackIconName, "ICON_UNIT_ROCK_BAND_PORTRAIT"};
		end
	end

	local iconName, prefixOnlyIconName, eraOnlyIconName, fallbackIconName = GetUnitPortraitIconNamesFromDefinition(owner, unitDef);
	return {iconName, prefixOnlyIconName, eraOnlyIconName, fallbackIconName, "ICON_"..unitDef.UnitType.."_PORTRAIT"};
end
...
function GetProductionInfoOfCity( pCity:table, productionHash:number )
	...
	elseif( districtDef ~= nil ) then
		...
		kIcons			= GetAkaneProductionDistrictIcons(districtDef.DistrictType);
	...
	elseif( unitDef ~= nil ) then
		...
		kIcons			= GetAkaneProductionUnitIcons(owner, unitDef)
